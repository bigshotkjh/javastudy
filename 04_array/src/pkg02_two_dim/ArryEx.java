...
public class ArryEx {
  ...
  public static void ex1() {
     ...
     *  2) new 자료형[행개수][] (<- 열개수 자리에 아무것도 안들어가. 열개수는 나중에 정하겠다는 것. 길이가 다른 1차원 배열 여러 개를 관리할 떄 사용.저장공간 낭비가 없고 더 좋아!!) 
     */
    // 길이가 동일한 1차원배열을 어러개 관리하는 방법
    int[][] arr1;
    arr1 = new int[3][2]; //3행 2열 테이블 (정확하게는)->즉! 길이가 2인 배열이 3개 있다.
    
    System.out.print(arr1[0][0]);
    System.out.print(arr1[0][1]);
    System.out.println();// 이렇게 하면 줄바꿈.
    ...
    // 길이가 다른 1차원 배열 여러 개를 관리할 떄 사용
    int[][] arr2;
    arr2 = new int[3][]; //행만 3개라고 정해 준거야!
    
    // 각 행(각 1차원 배열)의 열 개수(각 1차원 배열의 길이)를 결정해야 사용할 수 있다.
    arr2[0] = new int[2];  // 1행의 열은 2개!!  
    arr2[1] = new int[3];  // 2행의 열은 3개!!
    arr2[2] = new int[4];  // 3행의 열은 4개!! 라고 정해 준거야!
  ...
  public static void ex2() {
    ...
    */
    ...
    for(int i = 0; i < arr1.length; i++) { //행 길이 만큼 돌린다.
      for(int j = 0; j < arr1[i].length; j++) { //각 1차원 배열의 길이 즉 각각의 열 길이 만큼 돌인다.
